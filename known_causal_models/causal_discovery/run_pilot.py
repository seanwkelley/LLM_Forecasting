...
import sys
import io
# Fix Windows console encoding for LLM responses with unicode
...
def call_llm(
    ...
    for attempt in range(max_retries):
        ...
        if response.status_code in (500, 502, 503, 504):
            wait = 10 * (2 ** attempt)
            print(f"  [{response.status_code} SERVER ERROR] Waiting {wait}s "
        ...
        try:
            ...
            print(f"  [API ERROR] Invalid JSON in HTTP response, retrying...")
            time.sleep(5)
            continue
...
def _format_accumulated_graph(accumulated_graph: dict) -> str:
    """Format the accumulated graph as human-readable text for prompts."""
    if not accumulated_graph:
        return "No edges discovered yet."
    lines = [f"Current causal graph ({len(accumulated_graph)} edges):"]
    for (src, tgt), info in sorted(accumulated_graph.items()):
        conf = info.get("confidence", "?")
        lines.append(f"  {src} -> {tgt} ({conf})")
    return "\n".join(lines)


def _merge_edge_updates(accumulated_graph: dict, edge_updates: list) -> None:
    """Merge edge_updates from model response into accumulated graph in-place.

    - 'confirmed' / 'partially confirmed': add or update the edge
    - 'disconfirmed': remove the edge if present
    """
    for update in edge_updates:
        ...
        if not src or not tgt:
            continue
        key = (src, tgt)
        # Models use different key names: "update", "status", "action"
        status = (update.get("update", "") or update.get("status", "")
                  or update.get("action", "")).lower()
        if "disconfirm" in status or "remove" in status or "reject" in status:
            accumulated_graph.pop(key, None)
        elif ("confirm" in status or "add" in status
              or "establish" in status or "new" in status):
...
def run_pilot(
    ...
    # Accumulated graph: {("from", "to"): {"confidence": ..., "reasoning": ...}}
    # Programmatically maintained from edge_updates so the model sees its own
    # growing graph each turn, rather than relying on it to maintain state.
    accumulated_graph = {}
    # Seed with initial confident edges from observation
    ...
    max_consecutive_duplicates = 5  # give up after 5 consecutive duplicates
    ...
    while completed_interventions < budget and consecutive_duplicates < max_consecutive_duplicates:
        ...
        intervention = Intervention(
            type=int_type,
            target=target,
            run_periods=3,  # Fixed: 3 periods per intervention for consistency
            description=int_spec.get("description", f"Intervention {step+1}"),
        )

        # Dedup check — retry without consuming budget
        ...
        # Feed result back into conversation and ask for full updated graph
        ...
        # Replace accumulated graph with model's full current_graph
        ...
        # Checkpoint after each intervention
        if output_dir:
            ...
            ckpt = {
                ...
                "intervention_keys": [str(k) for k in all_intervention_keys],
                "llm_calls": llm_calls,
            }
    ...
    if consecutive_duplicates >= max_consecutive_duplicates and verbose:
        print(f"\n  [STOPPED] {max_consecutive_duplicates} consecutive duplicates — model has exhausted unique interventions")
...
def run_conflict_pilot(
    ...
    # Accumulated graph (same pattern as market pilot)
    accumulated_graph = {}
    for e in observation_response.get("confident_edges", []):
        key = (e.get("from", ""), e.get("to", ""))
        accumulated_graph[key] = {
    ...
    while completed_interventions < budget and consecutive_duplicates < max_consecutive_duplicates:
        ...
        intervention = Intervention(
            type=int_type,
            target=target,
            run_periods=3,  # Fixed: 3 periods per intervention
            description=int_spec.get("description", f"Intervention {step+1}"),
        )
        ...
        graph_text = _format_accumulated_graph(accumulated_graph)
        var_list = ", ".join(CONFLICT_VARIABLES)
        update_prompt = (
            f"GRAPH UPDATE (do NOT propose a new intervention — just update your graph).\n\n"
            f"INTERVENTION RESULT:\n{result_text}\n\n"
            f"YOUR CURRENT CAUSAL GRAPH:\n{graph_text}\n\n"
