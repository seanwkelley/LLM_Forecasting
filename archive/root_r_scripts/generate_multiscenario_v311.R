# Generate Multi-Scenario Dataset with v3.11 Sequential Per-Agent Proposals
# ==================================================================
#
# v3.11 changes from v3.10:
#   - Each domain expert makes a SEPARATE LLM call (not all in one prompt)
#   - Sequential ordering: military -> intelligence -> economic -> diplomatic
#   - Later agents see prior agents' proposals (inter-agent conditioning)
#   - Dynamic conditional guidance based on scenario parameters
#   - Presidential approval unchanged (sees all proposals, approves/vetoes)
#
# These changes create genuine inter-agent interaction for PID to detect.
#
# Outputs to outputs/multiscenario_v311/ to preserve v3.10 data for comparison.
# Uses same LHS parameters (seed=42) as original for direct comparison.
#
# Usage:
#   Rscript generate_multiscenario_v311.R [n_scenarios]
...
cat(rep("=", 70), "\n", sep="")
cat("MULTI-SCENARIO DATASET GENERATION (v3.11 Sequential Per-Agent Proposals)\n")
cat(rep("=", 70), "\n\n", sep="")
...
main <- function() {
  ...
  output_dir <- "outputs/multiscenario_v311"
  ...
  # Run simulations
  cat("\n", rep("=", 70), "\n")
  cat("RUNNING SIMULATIONS (v3.11 Sequential Per-Agent Proposals)\n")
  cat(rep("=", 70), "\n")
  ...
  cat("\n", rep("=", 70), "\n")
  cat("DATASET GENERATION COMPLETE (v3.11)\n")
  cat(rep("=", 70), "\n")
  ...
  cat(sprintf("  python forecasting/run_pid_emergence_analysis.py --data-dir %s --output-dir experiment_results/pid_analysis_v311\n", output_dir))
  cat(rep("=", 70), "\n")
}
