"""
Large-scale comparison: Personalized vs Generic agents (N=100 each)

Provides statistically significant test of whether cognitive profiles improve
...
print("="*80)
print("LARGE-SCALE COMPARISON: PERSONALIZED vs GENERIC")
print("="*80)
...
print(f"  Sampled 100 diverse personas")
print()
...
print("="*80)
print("CONDITION 1: GENERIC AGENTS")
print("="*80)
...
# ============================================================================
# CONDITION 2: PERSONALIZED AGENTS (100 predictions)
# ============================================================================

print("\n" + "="*80)
print("CONDITION 2: PERSONALIZED AGENTS")
print("="*80)

start_time = datetime.now()

def predict_personalized(faction, prompt_func, personas, actual_actions=None):
    """Run personalized predictions in parallel."""
    predictions = []
...
Output ONLY valid JSON in the exact format specified."""
        ...
        try:
            ...
            return {
                ...
                'bayesian_updating': persona.bayesian_updating_skill
            }
        except Exception as e:
            return {
                'agent_id': agent_id,
...
personalized_time = (datetime.now() - start_time).total_seconds()

print("\n" + "="*80)
print("PERSONALIZED RESULTS (N=100):")
print("="*80)
print(f"\nNovaris:")
print(f"  F1:        {personalized_novaris_metrics['mean_f1']:.3f} ± {personalized_novaris_metrics['std_f1']:.3f}")
print(f"  Precision: {personalized_novaris_metrics['mean_precision']:.3f} ± {personalized_novaris_metrics['std_precision']:.3f}")
print(f"  Recall:    {personalized_novaris_metrics['mean_recall']:.3f} ± {personalized_novaris_metrics['std_recall']:.3f}")
print(f"  Jaccard:   {personalized_novaris_metrics['mean_jaccard']:.3f}")

print(f"\nTethys:")
print(f"  F1:        {personalized_tethys_metrics['mean_f1']:.3f} ± {personalized_tethys_metrics['std_f1']:.3f}")
print(f"  Precision: {personalized_tethys_metrics['mean_precision']:.3f} ± {personalized_tethys_metrics['std_precision']:.3f}")
print(f"  Recall:    {personalized_tethys_metrics['mean_recall']:.3f} ± {personalized_tethys_metrics['std_recall']:.3f}")
print(f"  Jaccard:   {personalized_tethys_metrics['mean_jaccard']:.3f}")

print(f"\nTime: {personalized_time:.1f}s")
...
print(f"\n{'Metric':<25} {'Generic':<15} {'Personalized':<15} {'Difference':<15} {'Effect':<10}")
print("-"*85)
...
print(f"{'Novaris F1':<25} {generic_novaris_metrics['mean_f1']:<15.3f} "
      f"{personalized_novaris_metrics['mean_f1']:<15.3f} {nov_f1_diff:+.3f} ({nov_f1_pct:+.1f}%)  {nov_f1_effect:<10}")
...
print(f"{'Novaris Recall':<25} {generic_novaris_metrics['mean_recall']:<15.3f} "
      f"{personalized_novaris_metrics['mean_recall']:<15.3f} {nov_recall_diff:+.3f}")
...
print(f"{'Tethys F1':<25} {generic_tethys_metrics['mean_f1']:<15.3f} "
      f"{personalized_tethys_metrics['mean_f1']:<15.3f} {tet_f1_diff:+.3f} ({tet_f1_pct:+.1f}%)  {tet_f1_effect:<10}")
...
print(f"{'Tethys Recall':<25} {generic_tethys_metrics['mean_recall']:<15.3f} "
      f"{personalized_tethys_metrics['mean_recall']:<15.3f} {tet_recall_diff:+.3f}")
...
print(f"{'Combined F1':<25} {generic_combined_f1:<15.3f} "
      f"{personalized_combined_f1:<15.3f} {combined_diff:+.3f} ({combined_pct:+.1f}%)  {combined_effect:<10}")
...
if combined_diff > 0.05:
    print(f"\n[OK] PERSONALIZED WINS: {combined_pct:.1f}% improvement")
    print(f"     Cognitive profiles improve strategic action prediction")
    ...
    print(f"     Personalization adds noise or distracts from core strategy")
    if nov_f1_diff < -0.05 and tet_f1_diff < -0.05:
        print(f"     Effect consistent across both factions")

else:
    print(f"\n[NEUTRAL] NO SIGNIFICANT DIFFERENCE: {combined_pct:+.1f}%")
    print(f"     Personalization neither helps nor hurts prediction accuracy")
    print(f"     Value may lie in diversity rather than accuracy")

print("\n" + "="*80)
print(f"Total time: {(generic_time + personalized_time)/60:.1f} minutes")
print(f"Total API calls: 400")
print("[OK] Large-scale comparison complete!")
