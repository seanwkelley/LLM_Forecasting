"""
Conflict PID Analysis — Run PID on conflict simulation results.

Takes output from run_conflict_sim.py (scenario JSON files) and runs
...
from conflict.pid_extraction import (
...
def run_pid_on_conflict_data(
    ...
    """Run PID analysis on conflict simulation results.
    ...
    agent_groups : dict or None
        ...
        E.g., {"novaris": ["krasnov", "volkov", "petrova", "morozov"],
               "tethys": ["marchetti", "bondar", "kovalenko"]}
    n_permutations : int
    ...
    if agent_groups:
        for group_name, group_agents in agent_groups.items():
            ...
            if verbose:
                print(f"\n{'='*60}")
                print(f"FACTION: {group_name} ({group_cols})")
                print(f"{'='*60}")
    ...
    # --- Cross-faction PID (novaris vs tethys) ---
    cross_faction_results = {}
    if agent_groups and len(agent_groups) >= 2:
        factions = list(agent_groups.keys())
        for i, faction_a in enumerate(factions):
            for faction_b in factions[i+1:]:
                agents_a = [a for a in agent_groups[faction_a] if a in agent_cols]
                agents_b = [a for a in agent_groups[faction_b] if a in agent_cols]
                ...
                cross_faction_results[f"{faction_a}_vs_{faction_b}"] = {
                    "pairwise": cross_only,
    ...
    return {
        ...
        "cross_faction": cross_faction_results,
...
def compare_llm_vs_baseline(
    ...
    """Compare PID results between LLM and baseline conflict runs.

    Parameters
    ----------
    llm_result : dict
        Output of run_pid_on_conflict_data() for LLM run.
    baseline_result : dict
        Output of run_pid_on_conflict_data() for baseline run.
...
def main():
    parser = argparse.ArgumentParser(
        description="Run PID analysis on conflict simulation results"
    )
    parser.add_argument(
    ...
    # Conflict agent groups by faction
    agent_groups = {
        "novaris": ["krasnov", "volkov", "petrova", "morozov"],
        "tethys": ["marchetti", "bondar", "kovalenko"],
    }

    print("=" * 70)
    print("CONFLICT PID ANALYSIS")
    ...
    # Run main analysis
    llm_result = run_pid_on_conflict_data(
    ...
    if args.baseline_dir:
        ...
        baseline_result = run_pid_on_conflict_data(
    ...
    # Faction-level EC
    for group_name, group_data in llm_result.get("groups", {}).items():
        summary[f"{group_name}_ec"] = group_data["emergence_capacity"]

    # Cross-faction EC
    for cross_name, cross_data in llm_result.get("cross_faction", {}).items():
        summary[f"cross_{cross_name}_ec"] = cross_data["emergence_capacity"]
    ...
    with open(output_dir / "conflict_pid_summary.json", "w") as f:
    ...
    if pairwise_data:
        ...
        pairwise_df.to_csv(output_dir / "conflict_pairwise_pid.csv", index=False)
